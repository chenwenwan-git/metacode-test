...
function toggle2() {
    // 渲染数据
    slideImg2.src = slideData2[i].url
    iconImg.src = slideData2[i].icon
    pText.innerHTML = slideData2[i].p
    hText.innerHTML = slideData2[i].h2
    gamenameText.innerHTML = slideData2[i].gamename
    line.innerHTML = slideData2[i].line

    // 把之前的响应bar去掉
    document.querySelector('.product .bar .active').classList.remove('active')
    // 给当前bar添加响应
    document.querySelector(`.product .bar span:nth-child(${i + 1})`).classList.add('active')
}
